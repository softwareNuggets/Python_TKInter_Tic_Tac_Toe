...
class TicTacToe:
    ...
    def play(self, i, j):
        ...
        if self.check_win():
            self.game_over = True
            if(self.current_player == playerX[0]):
                self.status_label.config(text=playerX[3],background=playerX[2])
                self.my_win_count += 1
                self.update_scoreboard()
            elif(self.current_player == playerO[0]):
                self.status_label.config(text=playerO[3],background=playerO[2])
    ...
    def play_computer(self):
        ...
        if self.board[1][1] == "":
            self.board[1][1] = "O"
            self.buttons[1][1].config(text=playerO[0])
            if self.check_win():
                self.game_over = True
                self.their_win_count += 1
                self.update_scoreboard()
                self.status_label.config(text="Computer wins!", background=playerO[2])
            elif self.check_draw():
                self.game_over = True
                self.status_label.config(text="It's a draw!",background="Orange")
            else:
                self.current_player = playerX[0]
                self.status_label.config(text="Player X's turn",background=playerX[2])
            return
...
if __name__ == '__main__':
    ...
    # [0]=display character, [1]=player turn, [2]=status background color, [3]=winner string
    playerX = ('X', "Player X's turn", "lightSkyBlue", 'Player X is winner')
    playerO = ('O', "Player O's turn", "lightGreen", 'Player O is winner')
